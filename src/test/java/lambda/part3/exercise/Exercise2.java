...
public class Exercise2 {
    ...
    private static class MapHelper<T> {
        ...
        public <R> MapHelper<R> map(Function<T, R> mapping) {
            //return MapHelper.from(source.stream().map(mapping).collect(Collectors.toList()));

            List<R> rSource = new ArrayList<>();
            for (T t : source) {
                rSource.add(mapping.apply(t));
            }
            return MapHelper.from(rSource);
        }
        ...
        public <R> MapHelper<R> flatMap(Function<T, List<R>> flatMapping) {
            List<R> rSource = new ArrayList<>();
            for (T t : source) {
                rSource.addAll(flatMapping.apply(t));
            }
            return MapHelper.from(rSource);
        }
    }
    ...
    public void mapEmployeesToCodesOfLetterTheirPositionsUsingMapHelper() {
        ...
        List<Integer> codes = MapHelper.from(employees)
                ...
                .flatMap(s -> {
                    ...
                    for (char c : s.toCharArray()) listChars.add(c);
                    return listChars;
                })
                .map(c -> (int) c)
                .getMapped();
